...
{ 
      echo '<html>'; 
      echo '<head>'; 
			echo '<TITLE>Station Météo -- L.T.P OZANAM -- Éric Laly</TITLE>'; 
			echo '<meta http-equiv="content-type" content="text/html; charset=UTF-8" >'; 
			echo '<meta name="author" content="Eric Laly" >'; 
      ...
      echo '<body>';
   		echo '<div id="conteneur">';
			echo '<div id="entete">';
				echo '<h1>Affichage des données météo</h1>';
			echo '</div>';
			echo '<div class="frame">';
				echo '<div class="gauche">';
					echo '<UL class="sommaire_vertical">';
						echo '<LI><A href="affichage_temperature_interieure.php">Température intérieure</A></LI>';
						echo '<LI><A href="affichage_hygrometrie_interieure.php">Hygrométrie intérieure</A></LI>';
						echo '<LI><A href="affichage_pression_atmospherique.php">Pression atmosphérique</A></LI>';
       	...
       	echo '<br>'; 
				...
				// on recherche les températures intérieures
 				$sql="SELECT date, valeur from \"donneesMeteo\" where (\"id_TypeDonneeMeteo\"='1') ORDER BY date DESC;";
				$resultat=pg_exec($db,$sql);
				$lignes=pg_numrows($resultat);
				echo ("$lignes températures intérieures<br><br>");
 				...
 				if ($lignes!= 0) 
				{
					...
					{ 
						...
						echo("<TR>");
							echo("<TD>{$resultat_requete->date}</TD>");
							echo("<TD>{$resultat_requete->valeur} °C</TD>");
						echo("</TR>");
					} 
					echo("</TABLE>");
				}	
